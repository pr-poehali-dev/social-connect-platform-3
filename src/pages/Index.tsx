...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 font-golos text-zinc-100">
      ...
      {achievementToast && (
        <div
          onClick={() => { openGame(achievementToast.game); setAchievementToast(null); }}
          ...
          <div className="relative bg-gradient-to-br from-zinc-900 to-zinc-950 border border-amber-500/40 rounded-2xl shadow-2xl shadow-amber-500/20 p-4 pr-10 max-w-sm overflow-hidden">
            ...
            <div className="relative flex items-center gap-3">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center text-3xl shadow-lg shadow-amber-500/40 flex-shrink-0 animate-bounce">
                {achievementToast.medal}
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Icon name="Trophy" size={12} className="text-amber-400" />
                  <span className="text-[10px] font-bold uppercase tracking-wider text-amber-400">
                    {achievementToast.isNew ? "Новое в Зале славы" : "Подъём в Зале славы"}
                  </span>
                </div>
                <h4 className="text-sm font-bold text-white truncate">«{achievementToast.game}»</h4>
                <p className="text-xs text-zinc-400">
                  {achievementToast.place === 1 ? "1-е место — золото!" : achievementToast.place === 2 ? "2-е место — серебро!" : "3-е место — бронза!"}
                </p>
              </div>
            </div>
          </div>
